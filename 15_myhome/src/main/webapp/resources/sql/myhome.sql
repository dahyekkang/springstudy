...
CREATE TABLE UPLOAD_T (
    ...
    CONTENTS        VARCHAR2(4000 BYTE) NULL            ,
    USER_NO         NUMBER              NULL            ,       -- 사용자가 탈퇴했을 때, 지우고 싶으면 NOT NULL주고 ON DELETE CASCADE  /  남기고 싶으면 NULL주고 ON DELETE SET NULL
    CREATED_AT      VARCHAR2(30 BYTE)   NULL            ,
    MODIFIED_AT     VARCHAR2(30 BYTE)   NULL            ,
    ATTACH_COUNT    NUMBER              DEFAULT 0       ,
    CONSTRAINT PK_UPLOAD PRIMARY KEY(UPLOAD_NO),
    CONSTRAINT FK_USER_UPLOAD FOREIGN KEY(USER_NO) REFERENCES USER_T(USER_NO) ON DELETE SET NULL            -- 사용자가 탈퇴해도 게시글은 남아있다
);
...
CREATE TABLE ATTACH_T (
    ...
    HAS_THUMBNAIL       NUMBER              NULL        ,       -- 썸네일이 있으면 1, 없으면 0
    UPLOAD_NO           NUMBER              NOT NULL    ,
    CONSTRAINT PK_ATTACH PRIMARY KEY(ATTACH_NO),
    CONSTRAINT FK_UPLOAD_ATTACH FOREIGN KEY(UPLOAD_NO) REFERENCES UPLOAD_T(UPLOAD_NO) ON DELETE CASCADE     -- 업로드 게시글이 지워지면 첨부 파일도 함께 삭제
);
...
--    첨부가 없는 게시글도 조회할 수 있도록 외부 조인을 사용함
--    작성자가 없는 업로드 게시글도 조회할 수 있도록 외부 조인을 사용함 (탈퇴한 사용자)
SELECT UP.UPLOAD_NO, UP.TITLE, UP.CONTENTS, UP.CREATED_AT, UP.MODIFIED_AT, COUNT(ATC.ATTACH_NO) AS 첨부파일수, USR.USER_NO, USR.EMAIL, USR.NAME
  FROM UPLOAD_T UP LEFT OUTER JOIN ATTACH_T ATC                 -- 첨부하지 않은 게시물도 조회
    ON UP.UPLOAD_NO = ATC.UPLOAD_NO LEFT OUTER JOIN USER_T USR       -- 탈퇴한 사용자의 업로드 게시물도 조회
...
SELECT A.UPLOAD_NO, A.TITLE, A.CONTENTS, A.CREATED_AT, A.MODIFIED_AT, A.ATTACH_COUNT, A.USER_NO, A.EMAIL, A.NAME
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY UPLOAD_NO DESC) AS RN,
          ...
          FROM UPLOAD_T UP LEFT OUTER JOIN USER_T USR       -- 탈퇴한 사용자의 업로드 게시물도 조회
...
-- 1. 블로그 목록 (사용자 - 블로그 조인)
-- 부모 : 일대다 관계에서 일(PK OR UNIQUE)    - 사용자
-- 자식 : 일대다 관계에서 다(FK)              - 블로그

-- 내부 : 사용자와 블로그에 모두 존재하는 데이터를 조인하는 방식
-- 외부 : 사용자가 없는 블로그도 모두 조인하는 방식 (불가능 - BLOG_T의 USER_NO가 NOT NULL)
...
SELECT A.BLOG_NO, A.TITLE, A.CONTENTS, A.USER_NO, A.HIT, A.IP, A.CREATED_AT, A.MODIFIED_AT, A.EMAIL
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY B.BLOG_NO DESC) AS RN, B.BLOG_NO, B.TITLE, B.CONTENTS, B.USER_NO, B.HIT, B.IP, B.CREATED_AT, B.MODIFIED_AT, U.EMAIL   -- USER_NO 는 누구의 USER_NO여도 상관없음
          FROM USER_T U INNER JOIN BLOG_T B             -- 부모 테이블 먼저 적는 게 성능에 좋음
...
-- 2) 블로그 상세 정보 조회      사용자 정보를 가져오기 위해 JOIN 사용
...
-- 3) 댓글 목록(수정 불가)
-- 블로그 - 댓글 : 일대다관계 (댓글이 달린 블로그 정보는 이미 상세보기에 모두 표시되어 있으므로 여기에선 조인할 필요가 없음)
-- 사용자 - 댓글 : 일대다관계 (댓글에 사용자 이름을 표시)
