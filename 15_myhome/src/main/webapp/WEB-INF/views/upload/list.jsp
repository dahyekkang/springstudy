...
<style>
  ...
  .upload_list {
    ...
    margin: 10px auto;
    display: flex;
    flex-wrap: wrap;    /* 알아서 낑겨넣는 display:flex의 성질을 무시하고 아랫줄로 넣게 해주는 속성 */
  }
  .upload {
    width: 300px;
    height: 300px;
    ...
    text-align: center;
    padding-top: 100px;
    margin: 20px 15px;    /* 위아래 20px, 좌우 15px */
  }
  .upload:hover {
...
<script>
  ...
  const fnScroll = () => {
	  ...
	  $(window).on('scroll', () => {
		  ...
		  if(timerId) {   // timerId가 undefined이면 false로 인식, timerId가 값을 가지면 true로 인식(null, undifined는 false로 인식함)
			  clearTimeout(timerId);
		  }
		  
		  timerId = setTimeout(() => {    // setTimeout 실행 전에는 timerId가 undefined 상태, setTimeout이 한 번이라도 동작하면 timerId가 값을 가짐
			  
			  let scrollTop = $(window).scrollTop();    // 스크롤바의 위치(스크롤된 길이)
			  let windowHeight = $(window).height();    // 화면 전체 크기
			  // 스크롤바의 전체 높이를 알아야 얼마나 내려왔는지 알 수 있다.
