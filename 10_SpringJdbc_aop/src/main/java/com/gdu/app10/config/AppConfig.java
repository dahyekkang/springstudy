...
@EnableAspectJAutoProxy   // AOP 동작을 허용한다. (Configuration이 있는 클래스에서 사용)
@Configuration
public class AppConfig {
  ...
  public TransactionInterceptor transactionInterceptor() {
    ...
    ruleBasedTransactionAttribute.setRollbackRules(Collections.singletonList(new RollbackRuleAttribute(Exception.class)));   // Exception 발생하면 rollback 하겠다.
    
    // 
    MatchAlwaysTransactionAttributeSource matchAlwaysTransactionAttributeSource = new MatchAlwaysTransactionAttributeSource();  // TransactionInterceptor를 만들기 위한 재료. TransactionInterceptor가 요구하는 타입으로 바꾸는 과정
  ...
  // Advice : 트랜잭션 동작을 언제 할 것인지 결정하는 스프링 인터페이스
  @Bean
